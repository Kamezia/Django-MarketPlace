...
def items(request):
    ...
    if query:
        items = items.filter(Q(name__icontains=query) | Q(name__icontains=query))
...
def new(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            item.save()
            return redirect('item:detail', pk=item.id) # the detail needs pk (one item) to show it to the user.
    else:
        form = NewItemForm
